...
public class EmailService {
    ...
    public EmailService(String imapHost, String smtpHost, String user, String password, String authorizedSender) {
        this.imapHost = imapHost; this.smtpHost = smtpHost;
    ...
    public String send(String to, String subject, String body) {
        ...
        } catch (Exception e) { return "{\"status\":\"error\", \"msg\":\"" + e.getMessage() + "\"}"; }
    }
    ...
    public String read(long uid) {
        try {
            return executeInInbox(inbox -> {
                ...
                if (m == null) return "{\"error\":\"Mensaje no encontrado\"}";
    ...
    public void startListener(ConversationManagerImpl agent) {
        Thread t = new Thread(() -> {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                } catch (Exception e) { try { Thread.sleep(10000); } catch (InterruptedException ex) { break; } }
            }
        }, "EmailListener");
    ...
    private void invokeIdleSafely(Folder folder) {
        try {
            ...
            m.invoke(folder);
        } catch (Exception e) { try { Thread.sleep(30000); } catch (InterruptedException ex) {} }
    }
